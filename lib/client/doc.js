...
Doc.prototype.ingestData = function(data) {
  ...
  // deserialize the data into a snapshot if the type is provided and
  //  supports serialization/deserializtion
  if( data.data && data.type && types[ data.type ] && types[ data.type ].deserialize ) {
    this.snapshot = types[ data.type ].deserialize( data.data );
  } else {
...
var tryCompose = function(type, data1, data2) {
  if (data1.create && data2.del) {
    ...
    // Hack to prevent composing a serializable ot type
    //  Here the assumption is that type.appy can use the
    //  create.data ( the initial_data ) as the snapshot
    //  without serializing or deserializing it.
    //
    //  create.data must be serializable via JSON.stringify
    //  so that it can be sent over to the server.
    //  
    //  So for now we will just prevent the compose 
    //  optimization from being applied to serializable ot types
    //                           ---> __________________
  } else if (data1.create && data2.op && !type.serialize) {
